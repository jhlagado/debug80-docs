...
stack would reach, but computed at build, placed at known addresses,
and incapable of overflow. On a 64K machine the difference in kind
...
bracketed call site propagates through a restore stub whose
instructions leave the failure flag untouched — a requirement the
lowering contract must carry.
...
`frames[level + 1]` down. The depth bound and its bytes are visible in
source rather than guarded at runtime — which is also this model's
stated weakness: the profile's declared stack bound for scalar
recursion is a build-time declaration, not a runtime guard, and a
profile that requires the guard must add it. The reference compiler
...
have (the shadow registers serve one level at 4 T per exchange; further
levels save to the stack), does its narrow work, and returns.
...
- **The ring buffer** — the producer index owned by the handler side,
  the consumer index owned by the task side, each index written by
  exactly one world. The natural shape for serial input.
...
single-instruction form or be guarded, and a membrane object wider than
a word needs a protocol, not a copy.
...
- **Good SP-tracked compilation:** two pushes, `CALL`/`RET`, cleanup,
  first access at 28 T and the rest at 7 T while HL holds the frame
  address. Roughly 130–170 T when the access pattern cooperates —
...
- worst-case local RAM is a build-time number, and stack overflow in
  the non-recursive program is impossible rather than guarded;
