...
- **AZM register contract controls:** the TEC-1G Project panel includes session-scoped Register Care and Contract Update controls. These map to launch-time AZM options, while restart remains read-only unless future contract-update application is explicitly enabled.
