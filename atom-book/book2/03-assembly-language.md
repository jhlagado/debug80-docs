...
Atom also has a host preparation stage. A leading `%INCLUDE` names another
source file and `%IF`, `%ELSE` and `%ENDIF` select source before the Z80 core
reads it. Book 1 defines the full boundary between host directives and native
assembly.
...
Another routine may declare its own `.LOOP` without a collision. Atom combines
the private spelling with its owning global scope while assembling. The period
is part of the source name and does not count toward the eight significant
characters allowed after it.
