...
This book walks that whole path once, from an empty folder to running hardware. You will install the extension, create a TEC-1G project, build and step through a starter program, read the panel that shows the emulated machine, find the files each build produces, and send the generated HEX file to a TEC-1G through CoolTerm.
