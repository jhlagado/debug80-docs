...
The subroutine needs a counter. It uses D, and D must be initialized to zero before the loop. Initializing D does not disturb B or C — `ld d, 0` only touches D. But the subroutine wraps the initialization in `push bc / ld d, 0 / pop bc` anyway, because it was not obvious that zeroing D would leave BC alone. Saving BC was cheaper than working it out.
