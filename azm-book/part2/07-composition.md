...
2. **No `.org` in the library** unless you are deliberately placing code at a fixed address (unusual in Part 2).
3. **Every exported routine gets AZMDoc** — same as Part 1 Chapter 12 and Part 2 Chapters 1–3.
...
Part 1 Chapter 12 introduced **`.asmi`** files: contract records only, no instructions:
...
Part 2 examples stay self-contained in RAM; `.asmi` matters when you wire the same libraries into hardware later.
