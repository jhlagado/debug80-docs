...
Part 1 Chapter 8 introduced the stack for `call` / `ret` and for `push` / `pop`. Part 1 Chapter 11 added the **IX frame** when a routine needs stack-backed locals that survive nested calls.
...
If you made a second recursive call before storing the first result, you would have the same problem with **HL** — the register used for 16-bit results in Part 2. Pattern:
...
The IX frame from Part 1 Chapter 11 is the structured way to hold those slots when a routine needs several locals that must survive multiple `call`s — for example, Towers of Hanoi with two recursive counts before combining. This chapter's factorial only needs one saved register pair; `push bc` is enough.
...
Recursive routines use the same AZMDoc shape as every other `@` entry (Part 1 Chapter 12):
