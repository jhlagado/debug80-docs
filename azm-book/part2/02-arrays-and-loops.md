...
Reserve uninitialized storage with layout types when you want self-documenting size (Part 1 Chapter 13):
...
`cp c` / `jr nc` uses the unsigned sense from Part 1: carry set means A ≥ C. B doubles as the running index. `$FF` means not found — a sentinel index, not a valid offset for an 8-element table.
...
Reload HL before the second call — `insertion_sort` returns HL equal to the base (in DE), but treating reload as mandatory habit matches Part 1's lesson about clobbered pointers.
