...
This chapter reviews layout types from Part 1 Chapter 13, applies them to field reads and writes through HL and IX, and builds a **ring buffer** — a fixed-size FIFO queue over a byte table — as the main worked example. The companion listing is [`examples/05_ring_buffer.asm`](examples/05_ring_buffer.asm).
...
No allocator, no linked list nodes — just bytes, offsets, and compare/branch. That is the Part 2 sweet spot: representation before algorithm, with every memory access visible.
...
Part 1 Chapter 12 introduced AZMDoc: semicolon comments with `;!` tags for register contracts. Part 2 algorithm routines should always carry them.
