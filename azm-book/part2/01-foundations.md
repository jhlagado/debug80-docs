...
No division opcode is required — only compare, subtract, and swap. That fits the Part 2 theme: the algorithm is visible instruction by instruction.

---

## Part 2 calling convention (16-bit)

Part 1 established informal conventions: HL for addresses, A for byte results, callee-save for BC/DE/HL when used as scratch. Part 2 adds a **16-bit family** used in this chapter and reused later unless a chapter says otherwise.
...
Every subroutine in this part should document its contract with AZMDoc (Part 1 Chapter 12). The analyzer can then flag a caller that keeps HL live across a call to `gcd_u16`, which clobbers DE and returns a new HL.
...
- Part 2 uses an explicit **16-bit convention** (HL, DE, return in HL) on top of Part 1 byte conventions.
