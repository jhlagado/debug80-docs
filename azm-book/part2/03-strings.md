...
AZM also accepts a string directive that appends the terminator for you (Part 1 Chapter 3):
...
Unless a routine says otherwise, Part 2 string routines use:
...
You do not need a print port for Part 2 — storing `"42", 0` in RAM and inspecting bytes after `halt` is enough proof.
