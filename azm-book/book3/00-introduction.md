...
You finished Book 2 with a complete picture of the Z80 and the AZM surface that keeps assembly honest: register conventions, register contracts, layout types and ops. Book 3 puts that toolkit to work on algorithms and small data structures — the programs you would write in a systems or embedded course, but without leaving flat assembly.
...
When something breaks, you check the invariant first, then the instruction sequence. That is the debugging habit Book 2 started; Book 3 assumes you will use it on every chapter.

---

## Workspace RAM

Book 2 used registers for almost everything. Algorithms with more than a few live values need scratch space that survives across calls or nested loops.
...
`.org` places labels at fixed addresses. `.ds` reserves bytes without initializing them — fine for temporaries you overwrite before reading. Named workspace beats anonymous stack tricks in early chapters; recursion (Chapter 6) will revisit the IX stack frame from Book 2 Chapter 11.
...
Callers depend on that contract. `azm --rc warn` compares callers to callees the same way Book 2 Chapter 12 demonstrated. Book 3 does not introduce a new documentation dialect.
