...
function resolves(target, fromFile) {
  ...
  if (publicAbs !== undefined && existsSync(publicAbs)) {
    if (statSync(publicAbs).isDirectory())
      return existsSync(path.join(publicAbs, "index.html"));
    return true;
  }

  // A directory link serves its index.
  if (existsSync(abs) && statSync(abs).isDirectory())
    return existsSync(path.join(abs, "index.md"));
  ...
  return (
    existsSync(abs) ||
    existsSync(`${abs}.md`) ||
    existsSync(path.join(abs, "index.md"))
  );
}
...
for (const cfg of [".vitepress/config.ts", ".vitepress/sidebar.generated.js"]) {
  ...
  for (const m of text.matchAll(/link:\s*'([^']+)'/g)) {
    ...
    checked++;
    if (!resolves(target, path.join(ROOT, "x.md")))
      problems.push({ file: cfg, target });
  }
}

console.log(
  `  ${files.length} markdown files, ${checked} internal links checked`,
);
